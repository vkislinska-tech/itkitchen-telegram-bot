...
ПРАВИЛА:
- Ввічливе вітання на початку ✨.
- Кожна відповідь має закінчуватися запитанням (наприклад: "Скільки років вашій дитині?").
- Якщо питають про малювання — впевнено розповідай про Digital Art.
- Пропонуй дзвінок Вікторії після 4-го повідомлення або коли клієнт прямо спитає про запис.
...
app.post('/', async (req, res) => {
    try {
        ...
        // 2. ЛОГІКА /START
        if (userText === '/start') { sessions[chatId] = []; }
        if (!sessions[chatId]) sessions[chatId] = [];

        // Захист від порушення черги ролей (User-Model-User)
        ...
        if (data.candidates && data.candidates[0]?.content?.parts?.[0]?.text) {
            ...
            sessions[chatId].pop(); // Видаляємо останній запит, якщо API не відповіло
